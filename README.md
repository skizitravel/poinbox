...
Each exception now shows severity, priority, PO, customer, reason, field/current value, confidence, age, owner, status, and created date. Edit users can open the PO, assign a task, resolve it, ignore it, or use bulk resolve/ignore for selected rows. The queue can be filtered by status, severity, customer, and sorted by severity, priority, oldest, or newest. **Open Next** jumps to the next actionable PO.

PO rows show an open exception count badge. PO detail also includes an **Exceptions** section, **Possible Duplicates**, **Source Evidence**, and an **Audit Trail** section. The audit trail combines saved PO changes, resolved/ignored exceptions, duplicate decisions, and extraction feedback corrections where available.

Exceptions are regenerated from the current PO state, and the app tries to resolve related exceptions automatically when the underlying issue is fixed. For example, saving a missing PO number resolves the related field task, adding missing master data refreshes master-data tasks, and correcting line values refreshes line exceptions.

## Source Evidence Review

The extraction path stores field-level evidence in `extraction_field_evidence` where available. Evidence includes the extracted value, source snippet, attachment filename, confidence, and optional location metadata such as Excel sheet/row or Word paragraph. Low-confidence fields show a compact **Source Evidence** disclosure near the editable field, and PO detail includes a broader **Source Evidence** panel for reviewing captured snippets.

PDF coordinate highlighting is not implemented yet. For now, the app stores page/snippet-style evidence where practical. Excel and Word extraction preserve enough row/paragraph context to make review easier without adding a heavy document viewer.

## Duplicate Candidate Review

The app still prevents exact duplicate records using provider message IDs and PO number/revision checks. It now also stores duplicate candidates in `po_duplicate_candidates` for messier real-world scenarios such as same PO number with conflicting totals, blank revisions, different revisions, or repeated attachment hashes.

Duplicate candidates create actionable exceptions and appear in PO detail. Users can open the existing PO and choose **Mark Duplicate**, **Keep Both**, **Link Revision**, or **Ignore**. These decisions are recorded in the audit trail and keep the system from silently booking the same PO twice.
...
- **Date Received** is system-populated from the email received timestamp, or from the upload/import timestamp when there is no provider email timestamp.
- AI extraction does not determine **Date Received**, and any `date_received` returned by a model is ignored before the PO is inserted.
- Customer requested dates remain extracted on PO lines as `requested_date`.
...
Analytics now also includes top customers, top exception reasons, inbox reliability by account, duplicate-prevention metrics, and CSV downloads for summary, exceptions, and corrections. These reports are intentionally SQLite-backed and compact so the MVP can show throughput and ROI without adding a BI stack.
